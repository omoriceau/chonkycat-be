...
build-products:
	@echo "Building SAM template (products only)..."
	sam build ProductsFunction -t template.local.yaml
	@echo "Installing dependencies in Lambda build directory..."
	@lambda_dir=".aws-sam/build/ProductsFunction"; \
	...
	if [ -f lambdas/ProductsFunction/requirements.txt ]; then \
		pip install -r lambdas/ProductsFunction/requirements.txt -t "$lambda_dir" --quiet; \
	fi
...
db-up:
	...
	while [ $attempt -lt $max_attempts ]; do \
		if docker exec chonkychonk-db pg_isready -U chonky_user -d chonkychonk &> /dev/null; then \
...
.PHONY: db-down
db-down:
	@echo "Stopping Docker PostgreSQL database..."
	docker compose down
...
help:
	...
	@echo "  make db-up       - Start Docker PostgreSQL only"
	@echo "  make db-down     - Stop Docker PostgreSQL"
